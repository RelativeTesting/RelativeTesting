...
class Z3Wrapper(object):
	...
	def _findModelInt(self):
		...
		while self.N <= 64:
			self.solver.push()
			(ret,mismatch) = self._findModel2Int()
			if (not mismatch):
				break
			self.solver.pop()
			self.N = self.N+8
